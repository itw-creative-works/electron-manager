...
- **Windows production**: EV USB token (self-hosted runner now) or cloud signing (future, pluggable via `EM_WIN_SIGN_STRATEGY`).
...
3. Set `.env` (on the runner):
   ```
   EM_WIN_SIGN_STRATEGY=self-hosted
...
- Check `build/entitlements.mac.plist` — EM ships a default that covers Electron's needs.
- If your app uses additional capabilities (camera, microphone, etc.), add the corresponding entitlements there.
...
- [`build/README.md`](../src/defaults/build/README.md) — what goes in `build/`
