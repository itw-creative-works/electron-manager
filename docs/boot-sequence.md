...
1b. **userData path append** — in dev (`!app.isPackaged`), appends ` (Development)` to `app.getPath('userData')` so dev session data, logs, and `electron-store` files don't collide with a production-installed copy of the same app on the same machine. **Must run before `storage.initialize()`** (which constructs `electron-store` against the path).
